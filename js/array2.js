...
console.log(listNumberQuadruple); // undefined -> foreach không dừng, không trả về dc, chỉ sử dụng trong DOM

// filtter -> sàng lọc cac phần tử trong mảng thỏa điều kiện nào đó
const listThreeTo = listNumber.filter((value) => value > 3);
console.log(listThreeTo); // [4, 5]

// some -> trả về true khi thỏa 1 điều kiện
const listSome = listNumber.some((value) => value > 3);
console.log(listSome); // true

// every -> trả về true khi thỏa tất cả điều kiện
const listEvery = listNumber.every((value) => value > 0);
console.log(listEvery); // true
const listEvery1 = listNumber.every((value) => value > 3);
console.log(listEvery1); // false

// reduce ->gom các phần tử từ trái qua phải theo trình tự 2 phần tử
const listNumber1 = [1, 2, 3, 4];
const totalNumber = listNumber1.reduce((a, b) => {
        console.log(a, b);
        return a + b;
    }, 0)
    // 0 + 1 -> 1 
