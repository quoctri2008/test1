...
function timeSince(date) {
    ...
    let timer = seconds / 2592000; // 1 tháng = 2592000
    if (timer > 1) {
        console.log(`${Math.floor(timer)} năm trước`);
        return;
    }
    let timer = seconds / 604800; //1 tuần = 604800
    if (timer > 1) {
        console.log(`${Math.floor(timer)} năm trước`);
        return;
    }
    let timer = seconds / 86400; // 1 ngày = 86400
    if (timer > 1) {
        console.log(`${Math.floor(timer)} năm trước`);
        return;
    }
    let timer = seconds / 3600; // 1 giờ = 3600
    if (timer > 1) {
        console.log(`${Math.floor(timer)} năm trước`);
        return;
    }
    let timer = seconds / 60; // 1 phút = 60
    if (timer > 1) {
