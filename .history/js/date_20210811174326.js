...
function timeSince(date) {
    ...
    let timer = seconds / 60 * 60 * 24 * 30; // 1 tháng = 2592000
    if (timer > 1) {
        console.log(`${Math.floor(timer)} năm trước`);
        return;
    }
    let timer = seconds / 60 * 60 * 24 * 7; //1 tuần = 604800
    if (timer > 1) {
        console.log(`${Math.floor(timer)} năm trước`);
        return;
    }
    let timer = seconds / 60 * 60 * 24; // 1 ngày = 86400
    if (timer > 1) {
        console.log(`${Math.floor(timer)} năm trước`);
        return;
    }
    let timer = seconds / 60 * 60; // 1 giờ = 3600
    if (timer > 1) {
        console.log(`${Math.floor(timer)} năm trước`);
        return;
    }
    let timer = seconds / 1 * 60; // 1 phút = 60
    if (timer > 1) {
...
timeSince("Wed Aug 11 2021 16:10:36 GMT+0700 (Indochina Time)");